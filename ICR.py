...
class ICR(scrapy.Spider):
    ...
    start_urls = ['https://icr-global.org/clinical-research-news/page/']
    ...
    def __init__(self, max_items=20, output_file='ICR_news.xlsx', *args, **kwargs):
    ...
    def start_requests(self):
        for page_number in range(1, 6):
            ...
            print(f"Requesting: {base_url}")  # Optional debug
            yield scrapy.Request(url=base_url, callback=self.parse)

    def parse(self, response):
        articles = response.xpath('//div[contains(@class, "fusion-column-wrapper") and contains(@class, "fusion-content-layout-column")]')
        print(f"Found {len(articles)} article blocks.")  # Debug print

        for article in articles:
            if self.items_scraped >= self.max_items:
                break

            title_elem = article.xpath('.//a[@class="awb-custom-text-color awb-custom-text-hover-color"]')
            title = title_elem.xpath('text()').get(default='').strip()
            link = title_elem.xpath('@href').get(default='').strip()
            date = article.xpath('.//span[@class="fusion-tb-published-date"]/text()').get(default='').strip()
    ...
    def parse_article_page(self, response):
        article = response.xpath('//div[contains(@class, "fusion-column-wrapper") and contains(@class, "fusion-content-layout-column")]')
        title = response.meta.get('article_title', '')
        date = response.meta.get('article_date', '')
        article_url = response.meta.get('article_link', '')
        
        paragraphs = []
        for p in response.xpath('//div[contains(@class, "fusion-content-tb")]//p'):
        ...
        drug_info = self.extract_drug_names(full_text)
        ...
        row_data = [
            ...
            ', '.join(drug_names) if isinstance(drug_names, list) else drug_names,
