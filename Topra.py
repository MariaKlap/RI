...
class Topra(scrapy.Spider):
    name = 'Topra'

    start_urls = ['https://www.topra.org/TOPRA/TOPRA_Member/News_Folder/news.aspx?WebsiteKey=ee1ef362-412e-4402-9d9e-43f5e7447f19&New_ContentCollectionOrganizerCommon=']
    ...
    def __init__(self, max_items=30, output_file='Topra.xlsx', *args, **kwargs):
    ...
    def match_drug_terms(self, text):
        if not text.strip():
            return []
    ...
    def parse(self, response):
        ...
        articles = response.xpath('//div[@class="BlockTLItem"]')
        ...
        for article in articles:
            ...
            title_elem = article.xpath('.//h4/a')
            title = title_elem.xpath('text()').get(default='').strip()
            link = title_elem.xpath('@href').get(default='').strip()
            date = article.xpath('.//span[@class="PublishDate"]/span/text()').get(default='').strip()
    ...
    def parse_article_page(self, response):
        ...
        Product_Typee = self.classify_product(full_text.lower())
        ...
        drug_names = self.match_drug_terms(combined_text)
        ...
        row_data = [
            ...
            language[0] if isinstance(language, list) else language
        ]
    ...
    def classify_product(self, text):
        ...
        text = text.lower()
        for Product_Typee, keywords in self.Product_TypeES.items():
            if any(keyword in text for keyword in keywords):
                return Product_Typee
        return 'Other'
