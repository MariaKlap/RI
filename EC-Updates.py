...
class EC(scrapy.Spider):
    ...
    def extract_drug_names(self, title, summary, full_text):
        combined_text = ' '.join([title, summary, full_text])
        normalized_text = re.sub(r'[^\w\s]', '', combined_text.lower())

        matched = [drug for drug in self.drug_terms_set if drug in normalized_text]
