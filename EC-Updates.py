...
class EC(scrapy.Spider):
    ...
    start_urls = ['https://health.ec.europa.eu/latest-updates_en?page=']
    ...
    def __init__(self, max_items=20, output_file='EC-Updates.xlsx', *args, **kwargs):
    ...
    def extract_drug_names(self, title, summary, full_text):
        """Match drug names as full phrases using word boundaries."""
        combined_text = ' '.join([title, summary, full_text]).lower()
        normalized_text = re.sub(r'[^\w\s\-()+]', '', combined_text)

        matched = []
        for drug in self.drug_terms_set:
    ...
    def start_requests(self):
        total_pages = 3  # or any number you want to crawl

        yield scrapy.Request(url='https://health.ec.europa.eu/latest-updates_en', callback=self.parse)


        for page_number in range(1, total_pages):
            url = f'https://health.ec.europa.eu/latest-updates_en?page={page_number}'
    ...
    def parse(self, response):
        ...
        articles = response.xpath('//div[@data-ecl-auto-init="ContentBlock"]')
        ...
        for article in articles:
            ...
            date = article.xpath('.//li/time/text()').get(default='').strip()
            #summary = article.xpath('.//p/span[2]/text()').get(default='').strip()
            summary = ""
    ...
    def parse_article_page(self, response):
        ...
        drug_names = self.extract_drug_names(title, summary, full_text)
