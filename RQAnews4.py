...
class RQAnewsSpider(scrapy.Spider):
    name = 'RQA4news'
    ...
    def __init__(self, max_items=30, output_file='RQA_news.xlsx', *args, **kwargs):
    ...
    def parse(self, response):
        """Main parsing method that follows article links for full content"""
        article_links = response.css('li h3 a::attr(href)').getall()
        
        for link in article_links[:self.max_items]:  # Limit to max_items
    ...
    def parse_article_page(self, response):
        ...
        date = article.css('p strong::text').get('')
        
        # More robust paragraph extraction that doesn't use :has()
        paragraphs = []
        for p in article.css('p'):
            
            # Skip paragraphs that contain the "Back to News" link
        ...
        # Still yield the item for other exporters if needed
        yield {
            'title': title,
            'summary': summary,
