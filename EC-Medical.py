...
class ECM(scrapy.Spider):
    name = 'EC'
    max_pages = 2
    max_items = 20
    output_file = 'EC_Medical.xlsx'
    start_urls = ['https://health.ec.europa.eu/medical-devices-sector/latest-updates_en?page=']
    ...
    def __init__(self, max_items=None, max_pages=None, output_file=None, *args, **kwargs):
    ...
    def closed(self, reason):
        """
        Called when the spider closes. Saves collected data to Excel file.
        
        Args:
            reason: Reason why the spider was closed
        """
    ...
    def start_requests(self):
        for page_number in range(self.max_pages):
            base_url = f"{self.start_urls[0]}{page_number}/"
            print(f"Requesting: {base_url}")  
            yield scrapy.Request(url=base_url, callback=self.parse)

    def parse(self, response):
        articles = response.xpath('//article[@class="ecl-content-item"]')
        ...
        for article in articles:
            ...
            title_elem = article.xpath('.//div[@class="ecl-content-block__title"]/a')
            title = title_elem.xpath('text()').get(default='').strip()
            link = title_elem.xpath('@href').get(default='').strip()
            date = article.xpath('.//time[@datetime]/text()').get(default='').strip()
            ...
            yield scrapy.Request(
                ...
                meta={
                    ...
                    'article_date': date,
                    'article_title': title,
                    'article_link': response.urljoin(link)
                }
            )


    def parse_article_page(self, response):
        """Parse individual article page to extract full content using XPath"""
        article = response.xpath('//div[@class="ecl"]') 
        
        title = response.meta.get('article_title', '')
        date = response.meta.get('article_date', '')
        article_url = response.meta.get('article_link', '')
        
        # Extract paragraphs as full text
        paragraphs = []
        for p in response.xpath('//div[@class="ecl"]//p | //div[@class="ecl"]//li'):
            text = ' '.join(p.xpath('.//text()[not(parent::button)]').getall()).strip()
        ...
        # Process any PDFs found on the page
        pdf_texts = []
        for pdf_link in response.xpath('//a[contains(@href, ".pdf")]/@href').getall():
            pdf_url = response.urljoin(pdf_link)
            pdf_content = self.extract_pdf_content(pdf_url)
            if pdf_content:
                pdf_texts.append(pdf_content)
                
        # Modified section: Only summarize PDF if detail text is <= 300 words
        if pdf_texts:
            combined_text = "\n\n".join(pdf_texts)
            if word_count > 300:
                summary = full_text  # Use full detail text if over 300 words
            else:
                summary = self.generate_summary(combined_text, word_limit=100)
        else:
            combined_text = full_text
            summary = self.generate_summary(combined_text, word_limit=100)
            
        doc_type = self.classify_document(combined_text.lower())
        product_type = self.classify_product(combined_text.lower())
        countries = self.detect_countries(combined_text.lower())
    ...
    def extract_pdf_content(self, pdf_url):
        """Download and extract text from PDF files (max 2 pages)"""
        try:
            ...
            self.logger.error(f"❌ Failed to process PDF {pdf_url}: {str(e)}")
            return None


    def generate_summary(self, text, word_limit=100):
        """Generate concise summary from full text with exactly 100 words"""
        ...
        for sentence in sentences:
            ...
            if word_count + len(words) <= word_limit:
                ...
                if remaining >= 3:  # Only add partial sentence if we can add meaningful content
                    partial_sentence = ' '.join(words[:remaining])
                    summary.append(partial_sentence + '...')
                    word_count = word_limit
                break
        
        # Join and ensure exactly 100 words
        summary_text = ' '.join(summary)
        final_words = summary_text.split()[:word_limit]
