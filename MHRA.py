...
class MHRA(scrapy.Spider):
    name = 'MHRA'

    start_urls = ['https://mhrainspectorate.blog.gov.uk/page/']
    ...
    def __init__(self, max_items=20, output_file='MHRA.xlsx', *args, **kwargs):
    ...
    def parse(self, response):
        ...
        articles = response.xpath('//main/div/article')
        ...
        for article in articles:
            ...
            title_elem = article.xpath('.//header/h2/a')
            title = title_elem.xpath('text()').get(default='').strip()
            link = title_elem.xpath('@href').get(default='').strip()
            date = article.xpath('.//header/div/time/text()').get(default='').strip()
            summary = article.xpath('.//div/p/text()').get(default='').strip()
